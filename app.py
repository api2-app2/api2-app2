...
BASE_URL_E    = os.getenv("BASE_URL_E", os.getenv("BASE_URL", "http://localhost:3037/v2/elections"))
# BASE_URL_E    = os.getenv("BASE_URL_E", os.getenv("BASE_URL", "http://127.0.0.1:15037/v2/elections"))
# House districts endpoint:
BASE_URL_D    = os.getenv("BASE_URL_D", "http://localhost:3037/v2/districts")
...
PRIMARY_DATE  = os.getenv("PRIMARY_DATE", "2025-11-04")   # not used here, but safe to align
GENERAL_DATE  = os.getenv("GENERAL_DATE", "2025-11-04")   # ← 11/04/25
...
OFFICES    = [x.strip().upper() for x in os.getenv("OFFICES", "M").split(",") if x.strip()]   # ← Mayor only
...
# Snapshot in ./temp (inside app folder)
ROOT_DIR   = os.path.dirname(os.path.abspath(__file__))
PARENT_DIR = ROOT_DIR                                # was: one folder up
TEMP_DIR   = os.path.join(ROOT_DIR, "temp")          # was: ../temp
os.makedirs(TEMP_DIR, exist_ok=True)

# === Overrides config ===
OVERRIDES_PATH = os.getenv("OVERRIDES_PATH", os.path.join(TEMP_DIR, "overrides.json"))

# Static asset roots now inside the app folder
FAVICONS_DIR = os.path.join(ROOT_DIR, "favicons")
HISTORIC_DIR = os.path.join(ROOT_DIR, "HistoricData")
TOPOJSON_DIR = os.path.join(ROOT_DIR, "topojson")
FONTS_DIR    = os.path.join(ROOT_DIR, "fonts")
IMAGES_DIR   = os.path.join(ROOT_DIR, "images")

# Where snapshots/cache are written (stays under ./temp)
CACHE_SNAPSHOT_PATH = os.getenv(
    "CACHE_SNAPSHOT_PATH",
    os.path.join(TEMP_DIR, "nyc_cache.json")
)
...
CACHE_SNAPSHOT_PATH = os.getenv(
    "CACHE_SNAPSHOT_PATH",
    os.path.join(TEMP_DIR, "nyc_cache.json")   # ← write to ../temp/nyc_cache.json
)
...
def _build_url(state: str, office: str, race_type: str) -> str:
    ...
    # ✅ Districts only for House; everything else (incl. M) is statewide elections
    base = BASE_URL_D if office_u == "H" else BASE_URL_E
